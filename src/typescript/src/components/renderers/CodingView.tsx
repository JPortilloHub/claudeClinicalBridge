...
function renderItem(item: any): string {
  ...
  if (item && typeof item === 'object') {
    return item.note || item.query || item.description || item.text
      || Object.values(item).find((v: any) => typeof v === 'string') as string
      || JSON.stringify(item);
  }
  return String(item);
}
...
function IssuesTab({ data }: Props) {
  const notes = (data?.coding_notes || []).filter((n: any) => n != null);
  const queries = (data?.queries_needed || []).filter((q: any) => q != null);
    ...
    <div className="r-tab-inner">
      {notes.length > 0 && (
        <div className="r-alert r-alert-info">
          <h5>Coding Notes</h5>
          <ul className="r-list">
            {notes.map((n: any, i: number) => <li key={i}>{renderItem(n)}</li>)}
          </ul>
        </div>
      )}
      {queries.length > 0 && (
        <div className="r-alert r-alert-warning">
          ...
          <ul className="r-list">
            {queries.map((q: any, i: number) => <li key={i}>{renderItem(q)}</li>)}
          </ul>
        </div>
      )}
    </div>
